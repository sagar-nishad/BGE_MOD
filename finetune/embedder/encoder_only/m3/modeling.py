...
class EncoderOnlyEmbedderM3Model(AbsEmbedderModel):
    ...
    # [File: modeling.py]
#
# PASTE THIS ENTIRE CORRECTED METHOD:
    ...
    def forward(
        ...
        # --- START OF FIX ---
        # We compute loss as long as queries AND passages are provided
        # (which is true for both training and evaluation)
        if queries is not None and passages is not None:
            ...
            # Only increment step during actual training
            if self.training:
                self.step += 1
        
        # --- END OF FIX ---
        # The original `else: loss = None` block is now removed.
        # print("LOSS LOSS LOSS" , EmbedderOutput(
        #     loss=loss,
        # ))
        return EmbedderOutput(
            loss=loss,
